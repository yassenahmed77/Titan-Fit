...
function Header() {
    ...
    const active = scrolled || searchBarOpen || (window.innerWidth >= 1024 && hoverd);
            ...
            <header onMouseEnter={() => setHoverd(true)} onMouseLeave={() => setHoverd(false)} ref={headerRef} className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${active || isNavigated ? 'bg-white text-black py-10' : 'bg-transparent py-7 text-white'}`}>
                <div className="container">
                    <div className='grid grid-cols-3 items-center'>
                        ...
                        <div ref={menuRef} className={`fixed top-0 left-0 w-[80%] h-screen bg-white transition-all duration-300 pt-[30px] px-6 z-50 lg:hidden ${listOpen ? "translate-x-0" : "-translate-x-full"}`} onClick={(e) => e.stopPropagation()}>
                            <ul className='flex gap-5 flex-col text-black'>
                                {list.map((item) => {
                                    return (
                                        <li key={item.name} className="transition-all duration-300 hover:-translate-y-1 font-bold p-2.5 border-b border-gray-300 hover:border-black cursor-pointer">
                                            <Link to={item.link} onClick={() => setListOpen(false)}>{item.name}</Link>
                                        </li>
                                    )
                                })}
                                <li className="transition-all duration-300 hover:-translate-y-1 font-bold p-2.5 border-b border-gray-300 hover:border-black cursor-pointer">
                                    <Link to="/profile" onClick={() => setListOpen(false)}>Profile</Link>
                                </li>
                            </ul>
                            <X className='cursor-pointer absolute top-2.5 right-2.5 text-black' onClick={() => setListOpen(false)} />
                        </div>
                        {/* Links */}
